...
public class StringDecomposer {
   ...
   public static boolean iterateFormatted(String pText2, int pSkip, Style pCurrentStyle, Style pDefaultStyle, FormattedCharSink pSink) {
      String pText = protect(pText2);
      int i = pText.length();
   ...
   private static String protect(String string) {
      if (!Extazyy.getModuleManager().get("StreamerMode").state || Mine.mc.player == null)
         return string;
      String me = Mine.mc.getUser().getName();
      if (string.contains(me))
         return string.replace(me, "0xFruzz");
