...
public class GlyphMap {
    ...
    public void destroy() {
        Extazyy.mc.getInstance().getTextureManager().release(this.bindToTexture);
        this.glyphs.clear();
        this.width = -1;
        this.height = -1;
        generated = false;
    ...
    public static void registerBufferedImageTexture(ResourceLocation i, BufferedImage bi) {
        try {
            ...
            @SuppressWarnings("DataFlowIssue") long ptr = image.getPointer();
            ...
            if (RenderSystem.isOnRenderThread()) {
                Extazyy.mc.getInstance().getTextureManager().register(i, tex);
            } else {
                RenderSystem.recordRenderCall(() -> Extazyy.mc.getInstance().getTextureManager().register(i, tex));
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
